...
```js
export class GasRangeEffect {
    constructor(viewer, lon, lat) {
        this.viewer = viewer
        this.viewModel = {
            emissionRate: 5,
            gravity: 2.0,//设置重力参数
    ...
    init() {
        ...
        var particleSystem = this.scene.primitives.add(
            new Cesium.ParticleSystem({
                image: require('../assets/images/smoke.png'),//生成所需粒子的图片路径
                //粒子在生命周期开始时的颜色
                startColor: Cesium.Color.GREENYELLOW.withAlpha(0.7),
                //粒子在生命周期结束时的颜色
                endColor: Cesium.Color.GREENYELLOW.withAlpha(0.3),
                //粒子在生命周期开始时初始比例
    ...
    //显隐粒子特效
    hideShowGasEffect(bool) {
        let _this = this;

        let list = this.viewer.entities.values.concat();
        list.forEach(e => {
            if (e.name && e.name.indexOf("gasRangeEffect") > -1) {
                e.show = bool;
            }
        });
...
let gasRangeEffect = new GasRangeEffect(viewer, lon, lat);
